...
def attn_call2_0(
    ...
    # (20,4096,77) or (40,1024,77)
    if hasattr(self, "store_attn_map"):
        ip_hidden_states, ip_attn_map = scaled_dot_product_attention(
            query, ip_key, ip_value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        from einops import rearrange
        self.attn_map = rearrange(ip_attn_map, 'b (h w) d -> b d h w', h=height) # (10,9216,77) -> (10,77,96,96)
    else:
...
def cross_attn_init():
    IPAttnProcessor.__call__ = attn_call
    IPAttnProcessor2_0.__call__ = attn_call # attn_call is faster


def register_cross_attention_hook(unet):
    for name, module in unet.named_modules():
        if not name.split(".")[-1].startswith('attn2'): # Only for cross attention layer (where attn1 is self-attention)
            continue
